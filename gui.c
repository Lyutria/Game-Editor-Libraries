...
typedef struct gui {
  ...
  union set { // A union to save memory space, since
    ...
    struct window { // A window container
      ...
      int resizable; // default is 0, 1 is true.
      struct {
        int x;
        int y;
      } offset;
...
void do_gui(char type[]) {
  if (animpos==0) {
    ...
    switch(this_e->type) {
      ...
      {
        // STATES
        const int section_size = (this_e->theme->window.height/3)/3;
        if(this_e->set.window.active==1) {
          ...
          if(this_e->set.window.state != 2 && this_e->set.window.state < 4) { // dragged
            ...
            switch (new_state-3) {
              case 1:
                if (width-xmouse > this_e->set.window.min.width) {
                  width = width - xmouse;
                  if (width < this_e->set.window.max.width) x += xmouse;
                }
                if (height-ymouse > this_e->set.window.min.height) {
                  height = height - ymouse;
                  if (height< this_e->set.window.max.height) y += ymouse;
                }
                break;
              case 2:
                if (height-ymouse > this_e->set.window.min.height) {
          ...
          if(this_e->redraw) {
            ...
            Image current_window = image_subimage(this_e->theme->window, 0,window_height*(this_e->set.window.state > 2 ? 0: this_e->set.window.state), window_width,window_height);
            ...
            if(new_state >= 3) {
              image_draw_3x3(this_e->theme->window_highlight);
            }
      ...
      case GUI_METER:
      {
        ...
        if(this_e->redraw) {
          ...
          int bar_length = (int)((double)width*((double)this_e->set.meter.value/(double)this_e->set.meter.max));
          erase(0,0,0,1);
...
void gui_mousedown() {
  ...
  switch(this_e->type) {
    ...
    case GUI_WINDOW:
      // Edge resizing
      if(hotspot(0,0,width,height) && !hotspot(5,5,width-3,height-3) && this_e->set.window.resizable) {
